...
4. Set `FOREIGHTKILLO_TELEGRAM_ENABLED=true`, the token, and a comma-separated chat allowlist.
...
Keep `FOREIGHTKILLO_DISCORD_ENABLED=false`, start the local application, verify `getMe`, then send a
paper-only health notification to the allowlisted test chat. Confirm that a denied chat fails
without retry. Disable the provider by setting `FOREIGHTKILLO_TELEGRAM_ENABLED=false` and restart.
